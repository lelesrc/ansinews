(function() {
  ...
  var feedColorMap = {
    '#c084fc': 'feed-0', '#7c3aed': 'feed-0',
    '#22d3ee': 'feed-1', '#0c6b80': 'feed-1',
    '#60a5fa': 'feed-2', '#2563eb': 'feed-2',
    '#fb923c': 'feed-3', '#c2410c': 'feed-3',
    '#4ade80': 'feed-4', '#166534': 'feed-4',
    '#f87171': 'feed-5', '#b91c1c': 'feed-5'
  };

  function getEffectiveTheme() {
    var stored = hasLocalStorage ? localStorage.getItem('ansinews-theme') : null;
  ...
  function toggleTheme() {
    ...
    if (hasLocalStorage) {
      localStorage.setItem('ansinews-theme', next);
    }
  ...
  function render(view) {
    ...
    const rowsHTML = view.items.map(function(item, index) {
      ...
      var feedClass = feedColorMap[item.css] || 'feed-1';

      return '<div class="row item-row' + selected + '" data-idx="' + index + '">'
        + '<span class="src ' + feedClass + '">' + src + '</span>'
    ...
    if (view.statusMsg) {
      ...
      statusHTML += '<span class="s-hints">' + esc(view.hintKeys).split('  ').map(function(h) {
        return '<span class="s-hint">' + h + '</span>';
      }).join('&ensp;&ensp;') + '</span>';
    }
  ...
  document.addEventListener('DOMContentLoaded', function() {
    ...
    document.addEventListener('click', function(event) {
      var themeButton = event.target.closest('.theme-btn');
      if (themeButton) {
        toggleTheme();
        return;
      }
