(function(root, factory) {
  ...
  const FEED_STYLES = [
    { ansi: '\x1b[35m', css: '#c084fc', cssDay: '#7c3aed' },
    { ansi: '\x1b[36m', css: '#22d3ee', cssDay: '#0c6b80' },
    { ansi: '\x1b[34m', css: '#60a5fa', cssDay: '#2563eb' },
    { ansi: '\x1b[33m', css: '#fb923c', cssDay: '#c2410c' },
    { ansi: '\x1b[32m', css: '#4ade80', cssDay: '#166534' },
    { ansi: '\x1b[31m', css: '#f87171', cssDay: '#b91c1c' }
  ];
  var defaultFeeds = [];
  ...
  function decodeXmlEntities(str) {
    return str
      ...
      .replace(/&quot;/g, '"')
      .replace(/&apos;/g, '\'')
      .replace(/&#(?:x([0-9a-fA-F]+)|(\d+));/g, function(_, hex, dec) {
  ...
  function parseOPML(xml) {
    ...
    var decodeXmlAttr = decodeXmlEntities;
  ...
  function createApp(platform) {
    ...
    function getHintKeys(includeQuit) {
      if (state.detail) {
        return '[↑↓ jk] nav  [⏎/o] open  [esc/q] back';
      }

      return '[↑↓ jk] nav  [←→ hl] feed  [/] filter  [⏎] detail  [o] open  [r] refresh'
        + (includeQuit ? '  [q] quit' : '');
    }
